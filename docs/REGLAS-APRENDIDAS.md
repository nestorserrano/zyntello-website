...
- el SSH de Bluehost no se cae por lo que el comando PESA sino por lo que DURA: `git rev-parse` pasa siempre y `php artisan` la tumba siempre, aunque el proceso en sí funcione perfectamente (lanzado desacoplado, el mismo comando responde sin una queja). Lo que dura se lanza con `(nohup ... &)` y se verifica leyendo su log; los archivos se suben con `pscp`, nunca metiendo el contenido en la línea de comando.
- vigilar un proceso no puede costar más conexiones que ejecutarlo: seguir un deploy leyendo su log cada 10 segundos con reintentos pidió ~270 conexiones SSH en 15 minutos, Bluehost bloqueó el acceso más de una hora y el script se quedó ciego a su propio despliegue —que iba bien— y lo dio por fallido. Una lectura por vuelta y una vuelta por minuto.
- que no se pueda LEER el registro de un trabajo y que el trabajo falle son cosas distintas, y llevan a decisiones opuestas: el silencio no es un fallo. Cuando el registro no confirma, se juzga por el efecto —el commit que quedó en producción, el estado HTTP—, nunca por no haber podido mirar. Y el commit desplegado se puede leer sin SSH: el bundle de Vite que referencia la página de login cambia con cada build.
- dos sesiones desplegando a la vez se pisan sin saberlo, y la red de seguridad de una puede ser el daño de la otra: una leyó como suyo el 503 que era el mantenimiento de la otra y estuvo a punto de levantarle la app en mitad de su copia de archivos —el 500 al usuario que el mantenimiento existe para evitar—, mientras su polling bloqueaba el SSH y dejaba a la otra sin aplicar sus migraciones. Producción acabó con el código nuevo y el esquema viejo, que es el peor de los dos estados. El deploy toma un candado antes de tocar nada, y quien lo encuentra se abstiene sin levantar la app.
- un candado sin caducidad bloquea los despliegues para siempre el día que un deploy muera sin soltarlo: se descarta pasado un plazo holgado frente a lo que dura el trabajo (media hora para un deploy de minutos) y se avisa de que se descartó.
- antes de atribuirse un cambio en producción, mirar el `git reflog` del servidor: dijo que el merge entró 25 minutos después del intento propio, y que el script que se creía responsable ni siquiera había llegado a ejecutarse —no existía su log—. Sin esa comprobación se da por propio el trabajo de otra sesión, y se documentan como ciertas unas causas que no lo son.
